...
public abstract class GamePanel extends JPanel implements ActionListener, MouseListener {
    ...
    private final int[] rows = {179, 280, 386, 497, 601};
    ...
    public GamePanel(MapConfig config) {
        ...
        topBar = ImageLoader.load("topbar.png");
        ...
        sunCounter = new SunCounter(); sunCounter.setBounds(110, 30, 90, 40); add(sunCounter);
        ...
        for (int y : rows) {
            mowers.add(new Lawnmower(200, y));
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawImage(topBar, 0, 0, getWidth(), 100, this);
        g.drawImage(counterBox, 115, 30, 80, 40, this);
        g.drawImage(counterBox, 220, 30, 80, 40, this);

        g.setFont(new Font("Arial", Font.BOLD, 20));
        g.setColor(Color.WHITE);
        g.drawString(String.valueOf(score), 240, 58);  // Adjust x and y to center text
        ...
        if (!gameOver && totalZombiesThisWave > 0) {
            int barX = 50, barY = 110, barWidth = 300, barHeight = 20;
...
// 🕓 Continue checking mower status on every frame
    ...
    public void tryPlacePeashooter(Point p) {
        ...
        if (cell != null && !isPlantOccupied(cell) && sunCounter.spendSun(100)) {
            plants.add(new Peashooter(cell.x - 10, cell.y - 20));
            occupiedCells.add(cell);
        } else {
    ...
    public void tryPlaceBeetroot(Point p) {
        ...
        if (cell != null && !isPlantOccupied(cell) && sunCounter.spendSun(125)) {
            plants.add(new Beetroot(cell.x, cell.y-10));
            occupiedCells.add(cell);
        } else {
